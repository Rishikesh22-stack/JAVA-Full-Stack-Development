...
public class InsertFlight extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ailine", "root", "Rishikesh22");
			Statement st = (Statement) conn.createStatement();

			int i = ((java.sql.Statement) st)
					.executeUpdate("insert into flight(name, Source, Dest, date, time, price)values('" + name + "','"
							+ Source + "','" + Dest + "','" + departure + "','" + time + "','" + price + "')");
